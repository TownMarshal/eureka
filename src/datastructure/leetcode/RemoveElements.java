...
public class RemoveElements {
     ...
     * @return
     */
    public ListNode removeElement2(ListNode head, int value) {
        ListNode sentinel = new ListNode(-1, head);
        ListNode p1 = sentinel;
...
//                p2 = p2.next;
     ...
     * @return 删除后的链表头
     */
    public ListNode removeElement3(ListNode p, int value) {
        if (p == null) {
            return null;
        }
        if (p.value == value) {
            return removeElement3(p.next, value);
        } else {
            p.next = removeElement3(p.next, value);
            return p;
        }
    }
